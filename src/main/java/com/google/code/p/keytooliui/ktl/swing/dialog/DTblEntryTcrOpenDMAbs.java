...
public abstract class DTblEntryTcrOpenDMAbs extends DTblEntryTcrOpenAbs 
{        
    // ---------
    // PROTECTED
    ...
    protected boolean _contentsOk_()
    {
        ...
        // is alias pointing to valid trusted certificate?
        ...
        if (intId > (super._boosIsValidTcr_.length-1))
            MySystem.s_printOutExit(this, strMethod, "intId > (super._boosIsValidTcr_.length-1)");
        
        if (super._boosIsValidTcr_[intId].booleanValue() != true)
        {
            String strBody = "Alias not pointing to valid trusted certificate:";
            strBody += "\n";
            strBody += "  ";
            strBody += "\"";
