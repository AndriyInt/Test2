...
public final class PTblEntryTcrSel extends PTblEntryTcrAbs
{
    // --------------------
    ...
    **/
    public Object getSelectionObject()
    {
        ...
        Object objSel = super._mtm_.getValueAt(intSelectionRow, TMEntryTcrSel.f_s_intColumnIdAlias);
    ...
    public PTblEntryTcrSel(
        ListSelectionListener lsnListenerParent
        )
    {
        super(PTblEntryTcrSel._f_s_intDeltaW);   
        super._mtm_ = new TMEntryTcrSel();
    ...
    private boolean _initColumnSize(JTable tbl)
    {
        ...
        for (int i=0; i<TMEntryTcrSel.s_intsColW.length; i++)
        {
            ...
            tcn.setPreferredWidth(Math.max(intHeaderWidth, TMEntryTcrSel.s_intsColW[i]));
            tcn.setMinWidth(Math.max(intHeaderWidth, TMEntryTcrSel.s_intsColW[i]));
        }
    ...
    private boolean _load(Object[][] objssData)
    {
        ...
        super._mtm_ = new TMEntryTcrSel(objssData);
    ...
    private boolean _setRendererStyled(JTable tbl)
    {
        ...
        tcnCur = tcm.getColumn(TMEntryTcrSel.f_s_intColumnIdElligible);
        tcnCur.setCellRenderer(tcrCur);
        
        // ----
        // boolean is key entry
        tcrCur = new TCRCheckedDot();
        tcnCur = tcm.getColumn(TMEntryTcrSel.f_s_intColumnIdIsKeyEntry);
        tcnCur.setCellRenderer(tcrCur);
        
        // ----
        // boolean is cert entry
        tcrCur = new TCRCheckedDot();
        tcnCur = tcm.getColumn(TMEntryTcrSel.f_s_intColumnIdIsCertEntry);
        tcnCur.setCellRenderer(tcrCur);
        
        // ----
        // boolean is self-signed cert
        tcrCur = new TCRCheckedDot();
        tcnCur = tcm.getColumn(TMEntryTcrSel.f_s_intColumnIdIsSelfSignedCert);
        tcnCur.setCellRenderer(tcrCur);
        
         // ----
        // boolean is trusted cert
        tcrCur = new TCRCheckedDot();
        tcnCur = tcm.getColumn(TMEntryTcrSel.f_s_intColumnIdIsTrustedCert);
        tcnCur.setCellRenderer(tcrCur);
